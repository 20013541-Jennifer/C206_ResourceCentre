...
public class ResourceCentreTest {
	...
	public void testRetrieveAllChromebook() {
		...
		// Test if Item list is not null but empty, so that can add a new item
				...
				// When items are loaned, the display changes accordingly
				testOutput = String.format("%-10s %-30s %-10s %-10s %-20s\n","CB0011", "My Google Chromebook 1st","No", "12/12/2020", "Mac OS");
				testOutput += String.format("%-10s %-30s %-10s %-10s %-20s\n","CB0012", "SAMSUNG Chromebook 4+","No", "12/12/2020", "Win 10");
				ResourceCentre.doLoanChromebook(chromebookList, "CB0011", "12/12/2020");
				ResourceCentre.doLoanChromebook(chromebookList, "CB0012", "12/12/2020");
				allChromebook= ResourceCentre.retrieveAllChromebook(chromebookList);
...
//		//Test that status is unavailable (false)
		for (int i =0; i<camcorderList.size();i++) {
			...
			assertEquals(camcorderList.get(i).getIsAvailable(),ResourceCentre.doLoanCamcorder(camcorderList, "CC0011", "12/12/2020"));
			}
		}
		...
		for (int i =0; i<camcorderList.size();i++) {
			if(camcorderList.get(i).equals(cc1)) {
			assertNotNull(camcorderList.get(i).getDueDate(),ResourceCentre.doLoanCamcorder(camcorderList, "CC0011", "12/12/2020"));
			}
		}
	...
	public void testDoLoanChromebook() {
		...
		//Given an empty list, after adding 1 item, the size of the list is 1
				...
				//Test that status is unavailable (true)
				for (int i =0; i<chromebookList.size();i++) {
					if(chromebookList.get(i).equals(cb1)) {
					assertEquals(chromebookList.get(i).getIsAvailable(),ResourceCentre.doLoanChromebook(chromebookList, "CB0011", "12/12/2020"));
					}
				}
				//test that duedate is not empty
				for (int i =0; i<chromebookList.size();i++) {
					if(chromebookList.get(i).equals(cb1)) {
					assertNotNull(chromebookList.get(i).getDueDate(),ResourceCentre.doLoanChromebook(chromebookList, "CB0011", "12/12/2020"));					}
				}
